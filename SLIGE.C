...
void dump_texture_lmp(dumphandle dh,texture_lmp *tl)
{
  ...
  *(int *)tbuf = texturecount;  tbuf += sizeof(int);
  ...
  for (ct=tl->custom_texture_anchor;ct;ct=ct->next) {
    ...
    *(short *)tbuf = 0;  tbuf += sizeof(short);
    *(short *)tbuf = ct->xsize;  tbuf += sizeof(short);
    *(short *)tbuf = ct->ysize;  tbuf += sizeof(short);
    *(short *)tbuf = 0;  tbuf += sizeof(short);
    *(short *)tbuf = 0;  tbuf += sizeof(short);
    for (patchcount=0,p=ct->patch_anchor;p;p=p->next) patchcount++;
    *(short *)tbuf = patchcount;  tbuf += sizeof(short);
    for (p=ct->patch_anchor;p;p=p->next) {
      *(short *)tbuf = p->x;  tbuf += sizeof(short);
      *(short *)tbuf = p->y;  tbuf += sizeof(short);
      *(short *)tbuf = p->number;  tbuf += sizeof(short);
      *(short *)tbuf = 1;  tbuf += sizeof(short);
      *(short *)tbuf = 0;  tbuf += sizeof(short);
    }
  }
...
void record_custom_patches(dumphandle dh, config *c, boolean even_unused)
{
  ...
  if (even_unused || FALSE) {
    ...
    p = pbuf;
    /* The picture header */
    *(short *)p = columns;         p += sizeof(short);   /* Width */
    *(short *)p = rows;            p += sizeof(short);   /* Height */
    *(short *)p = (columns>>1)-1;  p += sizeof(short);   /* Width offset */
    *(short *)p = rows-5;          p += sizeof(short);   /* Magic */
    /* The pointers to the columns */
    for (i=0;i<columns;i++) {
      *(int *)p = 8 + 4 * (columns) + i * (rows+5);  p += sizeof(int);
    }
